module ModuleAutomata (generateCellularAutomata, toString, toPrettyString, getHashCode) where
...
toPrettyString = unlines . map asciiArt
    where
        asciiArt row = map (\cell -> if cell == 0 then '□' else '■') row


getHashCode :: CellularAutomata -> Int
getHashCode = sum . map sum
